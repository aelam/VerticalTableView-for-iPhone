...
#ifdef DEBUG
	#define NIF_TRACE(fmt, ...) _NIF_LOG(@" [LINE:%d] %s " fmt,__LINE__,__FUNCTION__, ##__VA_ARGS__)
#else
...
#define NIF_INFO(fmt, ...)  _NIF_LOG(@"- %s [LINE:%d]" fmt,__FUNCTION__,__LINE__, ##__VA_ARGS__)
#define NIF_ERROR(fmt, ...) _NIF_LOG(@"! %s [LINE:%d]" fmt,__FUNCTION__,__LINE__, ##__VA_ARGS__)
